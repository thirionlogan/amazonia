...
describe("Endpoints", () => {
  ...
  describe("PATCH /item/:id", () => {
    ...
    const updatedItemReserve = {
      reserve: true,
    };

    it("should respond with a 200 and a message if name is updated", async () => {
      const response = await request(app)
        .patch("/item/5")
        .send(updatedItemName);
      expect(response.statusCode).toBe(200);
      expect(response.body.message).toBe("Item 5 has been updated");
    });
    it("should respond with 500 if it could not update name", async () => {
      const response = await request(app)
        .patch("/item/500")
        .send(updatedItemName);
      expect(response.statusCode).toBe(500);
    ...
    it("should respond with a 200 and a message if reserve is updated", async () => {
      const response = await request(app)
        .patch("/item/9")
        .send(updatedItemReserve);
      expect(response.statusCode).toBe(200);
      expect(response.body.message).toBe("Item 9 has been updated");
    });
    it("should respond with 500 if it could not update reserve", async () => {
      const response = await request(app)
        .patch("/item/256")
        .send(updatedItemReserve);
      expect(response.statusCode).toBe(500);
